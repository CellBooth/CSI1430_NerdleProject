...
SDL_Plotter::SDL_Plotter(int r, int c){
	row = r;
	col = c;
...
char SDL_Plotter::getKey(){
	...
	if((!currentKeyStates[SDL_SCANCODE_LSHIFT] &&
        !currentKeyStates[SDL_SCANCODE_RSHIFT])
       &&(currentKeyStates[SDL_SCANCODE_EQUALS])) {
            key = '=';
	}

	//cases for shifts and multiplication vs 8
	if(currentKeyStates[SDL_SCANCODE_LSHIFT] ||
       currentKeyStates[SDL_SCANCODE_RSHIFT]) {
	...
	if((!currentKeyStates[SDL_SCANCODE_LSHIFT] &&
        !currentKeyStates[SDL_SCANCODE_RSHIFT])
       &&(currentKeyStates[SDL_SCANCODE_8])) {
            key = '8';
	}
   ...
   if(currentKeyStates[SDL_SCANCODE_BACKSPACE]) key = '\b';
	...
	if(currentKeyStates[SDL_SCANCODE_RETURN]) key = SDL_SCANCODE_RETURN; //Enter from main keyboard
	   if(currentKeyStates[SDL_SCANCODE_KP_ENTER]) key = SDL_SCANCODE_RETURN; //Enter from keypad
